...
func NewQFileInfo2(file string) *QFileInfo {
	...
	g := newQFileInfo(QFileInfo_new2(file_ms))
	g.isSubclass = true
	return g
}
...
func NewQFileInfo3(file *QFileDevice) *QFileInfo {
	g := newQFileInfo(QFileInfo_new3(file.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQFileInfo4(dir *QDir, file string) *QFileInfo {
	...
	file_ms.len = size_t(len(file))
	defer free(unsafe.Pointer(file_ms.data))
	g := newQFileInfo(QFileInfo_new4(dir.cPointer(), file_ms))
	g.isSubclass = true
	return g
}
...
func NewQFileInfo5(fileinfo *QFileInfo) *QFileInfo {
	g := newQFileInfo(QFileInfo_new5(fileinfo.cPointer()))
	g.isSubclass = true
	return g
}
