...
func NewQNtfsPermissionCheckGuard() *QNtfsPermissionCheckGuard {
	g := newQNtfsPermissionCheckGuard(QNtfsPermissionCheckGuard_new())
	g.isSubclass = true
	return g
}
...
func NewQFile3(parent *QObject) *QFile {
	g := newQFile(QFile_new3(parent.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQFile4(name string, parent *QObject) *QFile {
	...
	name_ms.len = size_t(len(name))
	defer free(unsafe.Pointer(name_ms.data))
	g := newQFile(QFile_new4(name_ms, parent.cPointer()))
	g.isSubclass = true
	return g
}
