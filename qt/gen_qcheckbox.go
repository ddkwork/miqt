...
func NewQCheckBox(parent *QWidget) *QCheckBox {
	g := newQCheckBox(QCheckBox_new(parent.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQCheckBox3(text string) *QCheckBox {
	...
	g := newQCheckBox(QCheckBox_new3(text_ms))
	g.isSubclass = true
	return g
}
...
func NewQCheckBox4(text string, parent *QWidget) *QCheckBox {
	...
	text_ms.len = size_t(len(text))
	defer free(unsafe.Pointer(text_ms.data))
	g := newQCheckBox(QCheckBox_new4(text_ms, parent.cPointer()))
	g.isSubclass = true
	return g
}
