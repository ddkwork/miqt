...
func NewQTextStream() *QTextStream {
	g := newQTextStream(QTextStream_new())
	g.isSubclass = true
	return g
}
...
func NewQTextStream2(device *QIODevice) *QTextStream {
	g := newQTextStream(QTextStream_new2(device.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQTextStream3(array []byte) *QTextStream {
	...
	g := newQTextStream(QTextStream_new3(array_alias))
	g.isSubclass = true
	return g
}
...
func NewQTextStream4(array []byte, openMode OpenMode) *QTextStream {
	...
	array_alias.data = (char)(unsafe.Pointer(&array[0]))
	array_alias.len = size_t(len(array))
	g := newQTextStream(QTextStream_new4(array_alias, openMode))
	g.isSubclass = true
	return g
}
