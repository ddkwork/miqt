...
func NewQBitmap2(param1 *QPixmap) *QBitmap {
	g := newQBitmap(QBitmap_new2(param1.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQBitmap3(w int, h int) *QBitmap {
	g := newQBitmap(QBitmap_new3((int)(w), (int)(h)))
	g.isSubclass = true
	return g
}
...
func NewQBitmap4(param1 *QSize) *QBitmap {
	g := newQBitmap(QBitmap_new4(param1.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQBitmap5(fileName string) *QBitmap {
	...
	g := newQBitmap(QBitmap_new5(fileName_ms))
	g.isSubclass = true
	return g
}
...
func NewQBitmap6(fileName string, format string) *QBitmap {
	...
	g := newQBitmap(QBitmap_new6(fileName_ms, format_Cstring))
	g.isSubclass = true
	return g
}
