...
func NewQTextEdit(parent *QWidget) *QTextEdit {
	g := newQTextEdit(QTextEdit_new(parent.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQTextEdit3(text string) *QTextEdit {
	...
	g := newQTextEdit(QTextEdit_new3(text_ms))
	g.isSubclass = true
	return g
}
...
func NewQTextEdit4(text string, parent *QWidget) *QTextEdit {
	...
	text_ms.len = size_t(len(text))
	defer free(unsafe.Pointer(text_ms.data))
	g := newQTextEdit(QTextEdit_new4(text_ms, parent.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQTextEdit__ExtraSelection(param1 *ExtraSelection) *QTextEdit__ExtraSelection {
	g := newQTextEdit__ExtraSelection(QTextEdit__ExtraSelection_new(param1))
	g.isSubclass = true
	return g
}
