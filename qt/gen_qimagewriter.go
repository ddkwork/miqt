...
func NewQImageWriter() *QImageWriter {
	g := newQImageWriter(QImageWriter_new())
	g.isSubclass = true
	return g
}
...
func NewQImageWriter2(device *QIODevice, format []byte) *QImageWriter {
	...
	g := newQImageWriter(QImageWriter_new2(device.cPointer(), format_alias))
	g.isSubclass = true
	return g
}
...
func NewQImageWriter3(fileName string) *QImageWriter {
	...
	g := newQImageWriter(QImageWriter_new3(fileName_ms))
	g.isSubclass = true
	return g
}
...
func NewQImageWriter4(fileName string, format []byte) *QImageWriter {
	...
	format_alias.data = (char)(unsafe.Pointer(&format[0]))
	format_alias.len = size_t(len(format))
	g := newQImageWriter(QImageWriter_new4(fileName_ms, format_alias))
	g.isSubclass = true
	return g
}
