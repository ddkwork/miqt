...
func NewQFileSystemWatcher() *QFileSystemWatcher {
	g := newQFileSystemWatcher(QFileSystemWatcher_new())
	g.isSubclass = true
	return g
}
...
func NewQFileSystemWatcher2(paths []string) *QFileSystemWatcher {
	...
	g := newQFileSystemWatcher(QFileSystemWatcher_new2(paths_ma))
	g.isSubclass = true
	return g
}
...
func NewQFileSystemWatcher3(parent *QObject) *QFileSystemWatcher {
	g := newQFileSystemWatcher(QFileSystemWatcher_new3(parent.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQFileSystemWatcher4(paths []string, parent *QObject) *QFileSystemWatcher {
	...
	paths_ma := struct_miqt_array{len: size_t(len(paths)), data: unsafe.Pointer(paths_CArray)}
	g := newQFileSystemWatcher(QFileSystemWatcher_new4(paths_ma, parent.cPointer()))
	g.isSubclass = true
	return g
}
