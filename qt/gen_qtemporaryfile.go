...
func NewQTemporaryFile() *QTemporaryFile {
	g := newQTemporaryFile(QTemporaryFile_new())
	g.isSubclass = true
	return g
}
...
func NewQTemporaryFile2(templateName string) *QTemporaryFile {
	...
	g := newQTemporaryFile(QTemporaryFile_new2(templateName_ms))
	g.isSubclass = true
	return g
}
...
func NewQTemporaryFile3(parent *QObject) *QTemporaryFile {
	g := newQTemporaryFile(QTemporaryFile_new3(parent.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQTemporaryFile4(templateName string, parent *QObject) *QTemporaryFile {
	...
	templateName_ms.len = size_t(len(templateName))
	defer free(unsafe.Pointer(templateName_ms.data))
	g := newQTemporaryFile(QTemporaryFile_new4(templateName_ms, parent.cPointer()))
	g.isSubclass = true
	return g
}
