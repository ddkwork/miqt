...
func NewQImageReader() *QImageReader {
	g := newQImageReader(QImageReader_new())
	g.isSubclass = true
	return g
}
...
func NewQImageReader2(device *QIODevice) *QImageReader {
	g := newQImageReader(QImageReader_new2(device.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQImageReader3(fileName string) *QImageReader {
	...
	g := newQImageReader(QImageReader_new3(fileName_ms))
	g.isSubclass = true
	return g
}
...
func NewQImageReader4(device *QIODevice, format []byte) *QImageReader {
	...
	g := newQImageReader(QImageReader_new4(device.cPointer(), format_alias))
	g.isSubclass = true
	return g
}
...
func NewQImageReader5(fileName string, format []byte) *QImageReader {
	...
	format_alias.data = (char)(unsafe.Pointer(&format[0]))
	format_alias.len = size_t(len(format))
	g := newQImageReader(QImageReader_new5(fileName_ms, format_alias))
	g.isSubclass = true
	return g
}
