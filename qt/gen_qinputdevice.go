...
func NewQInputDevice() *QInputDevice {
	g := newQInputDevice(QInputDevice_new())
	g.isSubclass = true
	return g
}
...
func NewQInputDevice2(name string, systemId int64, typeVal DeviceType) *QInputDevice {
	...
	g := newQInputDevice(QInputDevice_new2(name_ms, (longlong)(systemId), typeVal))
	g.isSubclass = true
	return g
}
...
func NewQInputDevice3(parent *QObject) *QInputDevice {
	g := newQInputDevice(QInputDevice_new3(parent.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQInputDevice4(name string, systemId int64, typeVal DeviceType, seatName string) *QInputDevice {
	...
	g := newQInputDevice(QInputDevice_new4(name_ms, (longlong)(systemId), typeVal, seatName_ms))
	g.isSubclass = true
	return g
}
...
func NewQInputDevice5(name string, systemId int64, typeVal DeviceType, seatName string, parent *QObject) *QInputDevice {
	...
	seatName_ms.len = size_t(len(seatName))
	defer free(unsafe.Pointer(seatName_ms.data))
	g := newQInputDevice(QInputDevice_new5(name_ms, (longlong)(systemId), typeVal, seatName_ms, parent.cPointer()))
	g.isSubclass = true
	return g
}
