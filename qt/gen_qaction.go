...
func NewQAction2(text string) *QAction {
	...
	g := newQAction(QAction_new2(text_ms))
	g.isSubclass = true
	return g
}
...
func NewQAction3(icon *QIcon, text string) *QAction {
	...
	text_ms.len = size_t(len(text))
	defer free(unsafe.Pointer(text_ms.data))
	g := newQAction(QAction_new3(icon.cPointer(), text_ms))
	g.isSubclass = true
	return g
}
...
func NewQAction4(parent *QObject) *QAction {
	g := newQAction(QAction_new4(parent.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQAction5(text string, parent *QObject) *QAction {
	...
	text_ms.len = size_t(len(text))
	defer free(unsafe.Pointer(text_ms.data))
	g := newQAction(QAction_new5(text_ms, parent.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQAction6(icon *QIcon, text string, parent *QObject) *QAction {
	...
	text_ms.len = size_t(len(text))
	defer free(unsafe.Pointer(text_ms.data))
	g := newQAction(QAction_new6(icon.cPointer(), text_ms, parent.cPointer()))
	g.isSubclass = true
	return g
}
