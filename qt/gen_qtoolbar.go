...
func NewQToolBar(parent *QWidget) *QToolBar {
	g := newQToolBar(QToolBar_new(parent.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQToolBar2(title string) *QToolBar {
	...
	g := newQToolBar(QToolBar_new2(title_ms))
	g.isSubclass = true
	return g
}
...
func NewQToolBar4(title string, parent *QWidget) *QToolBar {
	...
	title_ms.len = size_t(len(title))
	defer free(unsafe.Pointer(title_ms.data))
	g := newQToolBar(QToolBar_new4(title_ms, parent.cPointer()))
	g.isSubclass = true
	return g
}
