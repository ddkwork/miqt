...
func NewQByteArrayMatcher() *QByteArrayMatcher {
	g := newQByteArrayMatcher(QByteArrayMatcher_new())
	g.isSubclass = true
	return g
}
...
func NewQByteArrayMatcher2(pattern []byte) *QByteArrayMatcher {
	...
	g := newQByteArrayMatcher(QByteArrayMatcher_new2(pattern_alias))
	g.isSubclass = true
	return g
}
...
func NewQByteArrayMatcher3(pattern QByteArrayView) *QByteArrayMatcher {
	g := newQByteArrayMatcher(QByteArrayMatcher_new3(pattern.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQByteArrayMatcher4(pattern string) *QByteArrayMatcher {
	...
	g := newQByteArrayMatcher(QByteArrayMatcher_new4(pattern_Cstring))
	g.isSubclass = true
	return g
}
...
func NewQByteArrayMatcher5(other *QByteArrayMatcher) *QByteArrayMatcher {
	g := newQByteArrayMatcher(QByteArrayMatcher_new5(other.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQByteArrayMatcher6(pattern string, length int64) *QByteArrayMatcher {
	pattern_Cstring := CString(pattern)
	defer free(unsafe.Pointer(pattern_Cstring))
	g := newQByteArrayMatcher(QByteArrayMatcher_new6(pattern_Cstring, (ptrdiff_t)(length)))
	g.isSubclass = true
	return g
}
