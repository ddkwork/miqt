...
func NewQReadWriteLock() *QReadWriteLock {
	g := newQReadWriteLock(QReadWriteLock_new())
	g.isSubclass = true
	return g
}
...
func NewQReadWriteLock2(recursionMode RecursionMode) *QReadWriteLock {
	g := newQReadWriteLock(QReadWriteLock_new2(recursionMode))
	g.isSubclass = true
	return g
}
...
func NewQReadLocker(readWriteLock *QReadWriteLock) *QReadLocker {
	g := newQReadLocker(QReadLocker_new(readWriteLock.cPointer()))
	g.isSubclass = true
	return g
}
...
func NewQWriteLocker(readWriteLock *QReadWriteLock) *QWriteLocker {
	g := newQWriteLocker(QWriteLocker_new(readWriteLock.cPointer()))
	g.isSubclass = true
	return g
}
