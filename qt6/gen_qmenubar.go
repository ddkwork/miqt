...
func NewQMenuBar(parent *QWidget) *QMenuBar {
	ret := newQMenuBar(C.QMenuBar_new(parent.cPointer()))
	ret.isSubclass = true
	return ret
}
...
func NewQMenuBar2() *QMenuBar {
	ret := newQMenuBar(C.QMenuBar_new2())
	ret.isSubclass = true
	return ret
}
...
func (this *QMenuBar) OnInitStyleOption(slot func(super func(option *QStyleOptionMenuItem, action *QAction), option *QStyleOptionMenuItem, action *QAction)) {
	...
	C.QMenuBar_override_virtual_InitStyleOption(unsafe.Pointer(this.h), C.intptr_t(cgo.NewHandle(slot)))
}
